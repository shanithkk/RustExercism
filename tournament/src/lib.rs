...
impl Team {
    pub fn new(name: String) -> Team {
        Team{name, wins: 0, points: 0, draw: 0, losses: 0, mp: 0}
    }
    ...
    pub fn print_score(&self) -> String {
        format!("{:width1$}| {:width2$} | {:width2$} | {:width2$} | {:width2$} | {:width2$}",
                   self.name, self.mp, self.wins, self.draw, self.losses, self.points, width1 = 31, width2 = 2)
    }
}
...
pub fn tally(match_results: &str) -> String {
    ...
    for game in match_results.split('\n') {
        ...
        match v[2] {
            "win" => {
                        let team1 = teams.entry(v[0].to_string()).or_insert_with(|| Team::new(v[0].to_string()));
                        team1.add_win();
                        let team2 = teams.entry(v[1].to_string()).or_insert_with(|| Team::new(v[1].to_string()));
                        team2.add_loss();
                    }
            "draw" => {
                        let team1 = teams.entry(v[0].to_string()).or_insert_with(|| Team::new(v[0].to_string()));
